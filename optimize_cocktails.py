#!/usr/bin/env python3
"""
Script para otimizar imagens de cocktails para mobile
...
def optimize_image(input_path, output_path, max_width=1200, quality=85):
    ...
    try:
        with Image.open(input_path) as img:
            ...
            size_msg = f"  {original_size/1024:.1f}KB → {new_size/1024:.1f}KB"
            print(f"{size_msg} ({reduction:.1f}% menor)")
...
def main():
    ...
    print("🍹 Otimizando imagens de cocktails...\n")
    ...
    for img_path in sorted(images):
        ...
        output_path = cocktails_dir / output_name
        
        if optimize_image(img_path, output_path):
            success_count += 1
            # Remove o arquivo JPG original após conversão
            try:
                img_path.unlink()
                print(f"  Removido arquivo original JPG\n")
            except Exception as e:
                print(f"  Aviso: não foi possível remover {img_path}\n")
